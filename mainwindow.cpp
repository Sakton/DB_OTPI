...
bool MainWindow::mySearchTableInDataBase(const QString &tableName)//ф-я лишняя. т.к. БД не даст создать таблицу с таким же именем!!
{
  QSqlQuery query;
...
bool MainWindow::copyInSaveTableIsNoNameTable(const QString &tableName) //имя таблицы не может начинаться с цифры
{
...
bool MainWindow::copyInSaveTableIsTableToNewNameTable(const QString &newTableName)
{
  if(this->createNewTable(newTableName))
    {
      QSqlQuery *query = new QSqlQuery(this->db);
      bool res = query->exec("INSERT INTO "
                             + newTableName
                             + " SELECT * FROM "
                             + this->tableName);
      if(!res)
...
bool MainWindow::saveFile(const QString &fileName)
{
  ...
  if(!file.open(QFile::WriteOnly | QFile::Text))
    {
      QMessageBox::warning(this, tr("Ошибка"), tr("не могу сохранить файл: %1:\n%2").arg(fileName, file.errorString()), QMessageBox::Ok);
      return false;
    }
...
void MainWindow::setCurrentFile(const QString &fileName)
{
  ...
  this->setWindowModified(false);  //функция предопределена, утанавливает флаг был ли изменен документ
...
bool MainWindow:: maybeSave()
{
  ...
  QMessageBox *box = new QMessageBox(QMessageBox::Warning, tr("Сохранение изменений"), tr("Сохранить изменения"),QMessageBox::Save | QMessageBox::Discard | QMessageBox::Cancel, this);
...
void MainWindow::createSortProxyModel(/*const QString &findText*/const QRegExp &patternSearch)
{
  ...
  this->sortModel->setFilterRegExp(patternSearch);
  this->sortModel->setFilterKeyColumn(-1);
  this->ui->tableViewUi->resizeRowsToContents();
}

QRegExp MainWindow::mySetPattern(const QString &searchText)
{
  return QRegExp(searchText);
}
...
void MainWindow::loadFile(const QString &fileName)
{
  ...
  if(!file.open(QFile::ReadOnly | QFile::Text))
    {
      QMessageBox::warning(this, tr("Ошибка открытия файла"), tr("Не могу открыть файл %1/n%2").arg(fileName, file.errorString()));
      return;
    }
  QString tbName;
  QTextStream in(&file);
...
void MainWindow::writeSettings()
{
  ...
  //this->pos(), this->size() - предопределены в qWidget
}

void MainWindow::readSettings()
{
  QSettings settings("РКК", "ОТПИ");
  QPoint pos = settings.value("pos", QPoint(200,200)).toPoint(); //начальные настройки положения
  QSize size = settings.value("size", QSize(400,400)).toSize(); //начальные настройки размера окна
  this->move(pos);
  this->resize(size);
}
...
{
  QString fileName = QFileDialog::getSaveFileName(this, tr("Сохранить проект"), this->curFile, tr("ОТПИ(*.otpi)"));
  if(fileName.isEmpty())
    {
      return false;
    }
  ...
  if(!this->mySearchTableInDataBase(newNameTable)){
      if(isUntitled && (this->curFile == "noName.otpi"))
        {
          if(this->copyInSaveTableIsNoNameTable(newNameTable)) //если таблица удачно переименована с нонейм, сохраняем файл!!!!!!!!!!!!!
            ...
            {
              QMessageBox::warning(this, tr("Ошибка"), tr("не могу ПЕРЕИМЕНОВАТЬ таблицу: %1").arg(newNameTable), QMessageBox::Ok);
              return false;
            }
        }
      else if(!isUntitled && (this->tableName != newNameTable))
        {
          if(this->copyInSaveTableIsTableToNewNameTable(newNameTable))
            ...
            {
              QMessageBox::warning(this, tr("Ошибка"), tr("не могу СКОПИРОВАТЬ таблицу: %1").arg(newNameTable), QMessageBox::Ok);
              return false;
            }
        }
    }
    ...
    {
      QMessageBox::warning(this, tr("Ошибка"), tr("такая таблица есть в БД: %1").arg(fileName), QMessageBox::Ok);
      return false;
    }
  return true;
}
...
{
  QString fileName = QFileDialog::getOpenFileName(this, tr("Открыть проект"), qApp->applicationDirPath(), tr("Проекты ОТПИ(*.otpi)"));
  if(!fileName.isEmpty())
    {
...
bool MainWindow::connectToDb()
{
  ...
  if(!this->db.open())
    {
      QMessageBox::warning(this, tr("ОШИБКА ПОДКЛЮЧЕНИЯ БД"), tr("Нет подключения к базе данных."), QMessageBox::Ok);
      return false;
    }
  else
    {
...
void MainWindow::createViewTableAndSqlModel(const QString &tableName)
{
  ...
  this->createSortProxyModel(QRegExp());
...
void MainWindow::on_actionPasteRow_triggered()
{
  ...
  if(numRowSelected == 0)
    ...
    {
      this->mySqlTableModel->insertRow(++numRowSelected);//numRowSelected++ - добавит над выделенной строкой
    }
}

void MainWindow::on_actionDelRow_triggered()
{
  QMessageBox *bx = new QMessageBox(QMessageBox::Warning, tr("УДАЛЕНИЕ ЗАПИСИ"), tr("Выделенная запись будет УДАЛЕНА!!!"), QMessageBox::Ok | QMessageBox::Cancel);
  ...
  if(!this->findDialogIsShow)
    {
      ...
      connect(this->findDialog, SIGNAL(sendFindTextFromFindDialog(QString)), this, SLOT(acceptFindTextFromFindDialog(QString)));
      connect(this->findDialog, SIGNAL(closeFindDialogSignal()), this, SLOT(acceptorCloseFindDialogSlot()));
...
bool MainWindow::on_actionConvertToCSV_triggered()
{
  if(this->mySqlTableModel->rowCount())
    {
      QString csvFileName = QFileDialog::getSaveFileName(this, tr("CSV FILE CONVERT"), tr(""), tr("ЭКСЕЛЬ(*.csv)"));
      if(csvFileName.isEmpty())
        {
          QMessageBox::warning(this, tr("НЕТ ИМЕНИ ФАЙЛА"), tr("Надо задать имя файла для сохранения"), QMessageBox::Ok | QMessageBox::Cancel);
          return false;
        }
      QFile csvFile(csvFileName);
      if(!csvFile.open(QFile::WriteOnly | QFile::Text))
        {
          QMessageBox::warning(this, tr("ОШИБКА ОТКРЫТИЯ ФАЙЛА"), tr("ОШИБКА ОТКРЫТИЯ ФАЙЛА ДЛЯ ЗАПИСИ, ИЛИ ЭТОТ ФАЙЛ ОТКРЫТ В ДРУГОЙ ПРОГРАММЕ"), QMessageBox::Ok);
          return false;
        }
      QStringList strList;
      ...
      for(int i = 0; i < this->ui->tableViewUi->horizontalHeader()->count(); ++i)
        {
          strList << "\"" + this->ui->tableViewUi->model()->headerData(i, Qt::Horizontal).toString() + "\"";
        }
      ts << strList.join(";") + "\n";
      for(int j = 0; j < this->ui->tableViewUi->verticalHeader()->count(); ++j)
        {
          strList.clear();
          strList << "\"" + this->ui->tableViewUi->model()->headerData(j, Qt::Vertical).toString() + "\"";
          for(int k = 0; k < this->ui->tableViewUi->horizontalHeader()->count(); ++k)
            {
              strList << "\"" + this->ui->tableViewUi->model()->data(this->ui->tableViewUi->model()->index(j,k), Qt::DisplayRole).toString() + "\"";
            }
          ts << strList.join(";") + "\n";
        }
      csvFile.close();
...
void MainWindow::on_tableViewUi_pressed(const QModelIndex &index)//вспомогательная функция, убрать на релиз-версии
{
  if(index.isValid()){} //чушь от желтых треугольников
}
...
void MainWindow::printPreview(QPrinter *myPrinter)
{
  ...
  if(model)
    {
      ...
      if(doc->pageCount() == 1)
        {
          myHtml += tr("<tr style=\'font:15px; font-style:italic; background-color:rgb(200,200,200);\';><th colspan=13>");
              ...
              if(!hHeader->isSectionHidden(i))
                {
                  if(i != 13)
                    {
                      myHtml += tr("<td align=\'center\' valign=\'middle\' style=\' font:10px; font-weight:bold;"
                                   "background-color:rgb(180,180,180);\'>");
                    }
                  else
                    {
                      myHtml += tr("<td align=\"center\" valign=\"middle\" width=150 style=\' font:10px; font-weight:bold;"
                                   "background-color:rgb(180,180,180);\'>");
                    }
  ...
  //        QString nameTable = tr("Оснастка, учтенная отделом ОТПИ КИС-416Ц, по состоянию на: ");
  //        nameTable += QDate::currentDate().toString("dd.MM.yyyy");
  ...
  //            {
  //                cursor = table->cellAt(0, i-1).firstCursorPosition(); //Возвращает первую действительную позицию курсора в этой клетке.
